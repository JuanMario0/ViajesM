...
import { COLORES, fmt, estimarDuracion, fetchRoute } from "@/lib/api";
...
export default function Mapa({ viajes }: { viajes: Viaje[] }) {
  ...
  useEffect(() => {
    ...
    const conRuta = viajes.filter(t => t.origenLat && t.destinoLat);

    Promise.all(conRuta.map(t =>
      fetchRoute(t.origenLat!, t.origenLng!, t.destinoLat!, t.destinoLng!)
        .then(coords => {
          if (!coords.length) { drawDirect(m, t, bounds); return; }
          ...
          L.circleMarker(coords[0], { radius: 6, color: COLORES[t.tipo], fillColor: "#fff", fillOpacity: 0.8, weight: 2 })
            .addTo(m).bindPopup(`<b>${t.origen}</b><br>${t.tipo} · ${t.fecha}`);
          L.circleMarker(coords[coords.length - 1], { radius: 6, color: COLORES[t.tipo], fillColor: "#fff", fillOpacity: 0.8, weight: 2 })
          ...
          const line = L.polyline(coords, { color: COLORES[t.tipo], weight: 3, opacity: 0.6 })
...
function drawDirect(m: L.Map, t: Viaje, bounds: L.LatLngTuple[]) {
  ...
  L.circleMarker(d, { radius: 6, color: COLORES[t.tipo], fillColor: "#fff", fillOpacity: 0.8, weight: 2 })
    .addTo(m).bindPopup(`<b>${t.destino}</b><br>${t.tipo} · ${t.fecha}`);
  const line = L.polyline([o, d], { color: COLORES[t.tipo], weight: 3, opacity: 0.6 }).addTo(m);
  line.on("mouseover", () => line.setStyle({ weight: 5, opacity: 1 }));
  line.on("mouseout", () => line.setStyle({ weight: 3, opacity: 0.6 }));
}
